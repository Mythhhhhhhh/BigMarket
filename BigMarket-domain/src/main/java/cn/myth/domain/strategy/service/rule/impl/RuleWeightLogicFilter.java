...
public class RuleWeightLogicFilter implements ILogicFilter<RuleActionEntity.RaffleBeforeEntity> {
    ...
    public RuleActionEntity<RuleActionEntity.RaffleBeforeEntity> filter(RuleMatterEntity ruleMatterEntity) {
        log.info("规则过滤-权重范围 userId:{} strategyId:{} ruleModel:{}", ruleMatterEntity.userId(), ruleMatterEntity.strategyId(), ruleMatterEntity.ruleModel());
        String userId = ruleMatterEntity.userId();
        Long strategyId = ruleMatterEntity.strategyId();
        ...
        if (CollectionUtils.isEmpty(analyticalValueGroup)) {
            return new RuleActionEntity<RuleActionEntity.RaffleBeforeEntity>()
        ...
        Optional<Long> nextValue = analyticalSortedKeys.stream()
        ...
        return nextValue.isPresent()
                ? new RuleActionEntity<RuleActionEntity.RaffleBeforeEntity>()
                    .data(new RuleActionEntity.RaffleBeforeEntity()
                        ...
                        .ruleWeightValueKey(analyticalValueGroup.get(nextValue.get())))
    ...
    private Map<Long, String> getAnalyticalValue(String ruleValue) {
        ...
        Map<Long, String> ruleValueMap = Arrays.stream(ruleValueGroups)
                .filter(StringUtils::isNotEmpty)
                .collect(Collectors.toMap(
                        ruleValueKey -> {
                            String[] parts = ruleValueKey.split(Constants.COLON); // 分割字符串以获取键和值
                            if (parts.length != 2) {
